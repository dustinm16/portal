...
class MediaMTXService(ManagedService):
    ...
    @staticmethod
    def _detect_version() -> tuple[int, int, int]:
        """Detect installed MediaMTX version. Returns (major, minor, patch)."""
        import re
        for path in ["/usr/local/bin/mediamtx", "mediamtx"]:
            try:
                ...
                m = re.search(r"v?(\d+)\.(\d+)\.(\d+)", result.stdout + result.stderr)
                if m:
                    return int(m.group(1)), int(m.group(2)), int(m.group(3))
            except Exception:
                pass
        return (0, 0, 0)  # unknown — use oldest-compatible keys

    async def generate_config_file(self) -> str:
        """Generate mediamtx.yml configuration, adapting key names to installed version."""
        cfg = self.get_merged_config()
        ...
        # Detect installed version to select correct config key names.
        # Keys that changed across versions:
        #   < v1.11: serverKey/serverCert, encryption, protocols, readBufferCount
        #  >= v1.11: rtspServerKey/rtspServerCert, rtspEncryption, rtspProtocols (readBufferCount removed)
        ver = self._detect_version()

        if ver >= (1, 11, 0):
            global_tls = ""  # global serverKey/serverCert removed; each section has its own
            rtsp_encryption_key = "rtspEncryption"
            rtsp_protocols_key = "rtspProtocols"
            rtsp_server_key_line = f"rtspServerKey: {key_path}\nrtspServerCert: {cert_path}"
            read_buffer_line = ""  # readBufferCount removed in v1.11
        else:
            global_tls = f"serverKey: {key_path}\nserverCert: {cert_path}\n"
            rtsp_encryption_key = "encryption"
            rtsp_protocols_key = "protocols"
            rtsp_server_key_line = ""  # covered by global serverKey/serverCert
            read_buffer_line = "readBufferCount: 512"

        yaml_content = f'''# MediaMTX Configuration — generated for v{ver[0]}.{ver[1]}.{ver[2]}
...
rtsp: yes
{rtsp_protocols_key}: [tcp]
{rtsp_encryption_key}: optional
rtspAddress: 127.0.0.1:{rtsp_port}
rtspsAddress: 127.0.0.1:{rtsps_port}
{rtsp_server_key_line}
