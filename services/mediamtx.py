...
# ALL services bound to localhost - external access via Portal Gateway (port 443)
...
paths:
  all_others:
    runOnNotReady: curl -s -X POST {portal_url}/api/stream/event -H Content-Type:application/json -d '{{"event":"disconnect","path":"$MTX_PATH"}}'
'''
        return yaml_content
    ...
    def get_stream_urls(self, stream_path: str, portal_host: str = "portal.dddvm.xyz") -> dict:
        """Get URLs for accessing a stream via Portal Gateway (port 443).
