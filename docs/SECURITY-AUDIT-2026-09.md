...
| F6 | `.lua` is writable through the config editor. For a Lua-scripting game server (e.g. Garry's Mod) a `files` grantee could write server-side Lua. | Game-sandbox-dependent, and `.lua` is a legitimate config format (Zomboid `SandboxVars.lua`, Factorio). Documented limitation. A dedicated Lua-scripting game would warrant a per-catalog "no `.lua` write" flag. |
