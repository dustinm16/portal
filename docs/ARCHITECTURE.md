...
- **Portal chrome + theme** - For `text/html` responses, `http_searxng_proxy` injects `theme.js` + `portal.css` + `searxng-portal.css` + `searxng-portal.js` before `</head>`. `searxng-portal.css` remaps SearXNG's ~105 `--color-*` vars onto the portal theme tokens; `searxng-portal.js` strips SearXNG's own `theme-*` `<html>` class, prepends the full portal navbar (identity from `/api/me`, SearXNG's About/Preferences folded in), and mounts the shared `ThemeSwitcher`. No SearXNG templates are forked. When `/search/` is iframed inside the embedded browser (`window.self !== window.top`), `searxng-portal.js` skips the navbar and routes external result-link clicks either into a new embedded tab through the proxy (`browser_mode` connections, via `window.parent.PORTAL_EMBEDDED_BROWSER`) or into a real top-level tab (single-origin connections).

### Theme system

`static/js/theme.js` is the single source: the `THEMES` list (28 entries), a synchronous pre-paint applier (`localStorage['portal-theme']` -> `html[data-theme]`, plus `html[data-theme-mode="light"]` for the light themes so `portal.css` can share the "black overlay -> dark-on-light" element tweaks), the `ThemeSwitcher` (swatch dropdown, mounts next to `.navbar-user` or an explicit `[data-theme-mount]`, repositions to stay on-screen), and a `storage`-event listener for cross-tab sync. Loaded in `<head>` before `portal.js` on every page — including `login.html`, `unauthorized.html` and `live.html`, which have no `portal.js` and get a standalone `[data-theme-mount]` corner control. The per-theme CSS variables live in `portal.css` (`:root` + `html[data-theme="<id>"]`), including `--text-primary-rgb` (used by `searxng-portal.css` for SearXNG's text-opacity blends). Three themes (`aurora`, `nebula`, `ember`) animate `body`'s gradient via `@keyframes theme-flow`; each sets an explicit `background-position: 50% 50%` so the global `prefers-reduced-motion` freeze lands on a mid-blend, not an edge colour.
