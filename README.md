...
### Game Servers
Deploy dedicated game servers straight from the admin panel. The **Game Servers** tab has a searchable catalog of ~75 games (Palworld, Valheim, Project Zomboid, Rust, ARK, CS2, Team Fortress 2, Arma 3, DayZ, Satisfactory, 7 Days to Die, and many more — admins can add their own entries too); pick a game, name the instance, and Portal runs the SteamCMD install, generates a systemd unit, and registers it as a managed service. From there you get start/stop/restart, live journal logs, a one-click **SteamCMD update** (with an "already up to date" check), a scoped **config-file editor** limited to that game's known settings files, and a **backup download** that packages the config and save/world files into a single `.tar.gz`. Everything runs under the same unprivileged account and install disk as a hand-rolled setup would.
