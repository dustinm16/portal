...
### Game Servers
Deploy dedicated game servers straight from the admin panel. The **Game Servers** tab has a searchable catalog (Palworld, Valheim, Project Zomboid, Rust, ARK, CS2, Satisfactory, 7 Days to Die, and more — admins can add their own entries); pick a game, name the instance, and Portal runs the SteamCMD install, generates a systemd unit, and registers it as a managed service. From there you get start/stop/restart, live journal logs, a one-click **SteamCMD update** (with an "already up to date" check), and a scoped **config-file editor** limited to that game's known settings files. Everything runs under the same unprivileged account and install disk as a hand-rolled setup would. Each server is a managed service, so you can **grant** a non-admin user `Control`, `Logs`, and/or `Files` and they'll run it and edit its configs from their own dashboard — nothing else.
